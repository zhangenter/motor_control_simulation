...
- dq 电流单环可选择 d 轴励磁或 q 轴转矩测试，支持锁轴测试与独立 d/q PI 参数
...
- 干扰按机械、负载、电气 Tab 分类：齿槽/摩擦、组合负载/惯量、PWM 开关纹波/死区/母线波动/反电动势谐波
- 编码器噪声、量化与延迟，以及可切换的速度估算和滤波链路
- 实时、暂停、单步、离线高速仿真和多次实验曲线叠加
- 曲线通道选择、十字游标读数、滚轮缩放、拖动平移和手动给定线拖动
- CSV 数据、PNG 曲线和 JSON 实验配置导入导出
- “实验”菜单支持新建、打开、保存和退出；“窗口”菜单可隐藏或显示实验参数、实时状态、运行日志和自定义控制器区域；“工具箱”提供三环 PID 计算器
- PID 计算器可按电流环、速度环、位置环载入电机参数、计算增益并写回当前实验
...
指令幅值的单位由“用户输入”决定。含位置外环的拓扑可选择位置输入（rad）或速度输入（rpm）；速度输入会在每个仿真步内换算并积分成位置指令。纯速度拓扑固定为 rpm，纯电流拓扑固定为 A。电流单环中可选择 d 或 q 测试轴，未激励轴的目标自动置零；锁定转子默认启用，以隔离机械运动和反电动势。速度指令、估算反馈、误差、曲线、CSV 数据及自定义控制器接口均统一使用 rpm；PMSM 物理方程内部会在需要时显式换算为 rad/s。
...
电气干扰采用适合连续 dq 仿真的等效模型：PWM 纹波在当前仿真步长可分辨的频率内折叠，死区由三相电流方向计算等效压降，母线波动按偏差与正弦纹波改变逆变器实际输出，反电动势基波仍由 PMSM 方程计算，干扰项只叠加可配置的非正弦谐波。dq 电压页可选看控制器指令和实际施加电压，CSV 同时记录母线电压及各电气扰动分量。

基础 PLL 和二阶状态观测器分别提供带宽与阻尼比参数；正交/带抗饱和 PLL 额外提供速度限幅，使用有界正交鉴相器和回算式 anti-windup；卡尔曼滤波提供加速度过程噪声参数，其位置测量噪声方差会根据编码器噪声与分辨率自动计算。界面只显示当前估算方法相关的参数。
...
也可以仅返回一个数值，此时它会作为 `vq`。`state` 提供 `id`、`iq`、`theta`、`omega`（rpm）、`torque` 和时间 `t`；`reference` 提供 `user_input` 原始用户输入、`command` 转换后的外环指令，以及 `position`、`speed`（rpm）、`current`、`id_ref`、`iq_ref` 内部目标；`params` 是一个可由脚本持续修改的字典。
